...
from hseling_api_poem_generator import boilerplate

from hseling_lib_poem_generator.process import process_data
from hseling_lib_poem_generator.query import query_data


app = Flask(__name__)
app.config['DEBUG'] = False
app.config['LOG_DIR'] = '/tmp/'
if os.environ.get('HSELING_API_POEM_GENERATOR_SETTINGS'):
    app.config.from_envvar('HSELING_API_POEM_GENERATOR_SETTINGS')
...
if not app.debug:
    ...
    file_handler = TimedRotatingFileHandler(os.path.join(app.config['LOG_DIR'], 'hseling_api_poem_generator.log'), 'midnight')
...
def healthz():
    ...
    return jsonify({"status": "ok", "message": "hseling-api-poem-generator"})
